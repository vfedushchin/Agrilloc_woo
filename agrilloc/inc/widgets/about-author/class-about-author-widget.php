 ...
 * @package Agrilloc
 */
if ( ! class_exists( 'Agrilloc_About_Author_Widget' ) ) {

	class Agrilloc_About_Author_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			...
			$this->widget_cssclass    = 'agrilloc widget-about-author';
			$this->widget_description = esc_html__( 'Display an information about selected user.', 'agrilloc' );
			$this->widget_id          = 'agrilloc_widget_about_author';
			$this->widget_name        = esc_html__( 'About Author', 'agrilloc' );
			$this->settings           = array(
				'title'  => array(
					'type'  => 'text',
					'value' => esc_html__( 'About Author', 'agrilloc' ),
				...
				'user_id' => array(
					...
					'options'          => false,
					'label'            => esc_html__( 'Select user to show', 'agrilloc' ),
				),
				'avatar_size' => array(
					'type'       => 'slider',
					'max_value'  => 512,
					...
					'value'      => 250,
					'step_value' => 1,
					'label'      => esc_html__( 'Author avatar size (set 0 to hide avatar, applied only for Gravatar)', 'agrilloc' ),
				),
				'avatar_img' => array(
					'type'               => 'media',
					'value'              => '',
					'multi_upload'       => false,
					'library_type'       => 'image',
					'upload_button_text' => esc_html__( 'Select image', 'agrilloc' ),
					'label'              => esc_html__( 'Custom avatar image (override default user avatar)', 'agrilloc' ),
				),
				'link' => array(
					'type'  => 'text',
					'value' => '',
					'label' => esc_html__( 'Link (leave empty to hide)', 'agrilloc' ),
				),
				'link_text' => array(
					'type'  => 'text',
					'value' => esc_html__( 'Read More', 'agrilloc' ),
					'label' => esc_html__( 'Link label', 'agrilloc' ),
				),
			);
		...
		public function get_users_list() {
			...
			$result = array( '0' => __( 'Select a user', 'agrilloc' ) );
		...
		public function get_author_avatar() {
			...
			if ( ! empty( $this->instance['avatar_img'] ) ) {
				$avatar_src = wp_get_attachment_image_src( intval( $this->instance['avatar_img'] ), 'agrilloc-author-avatar' );
...
add_action( 'widgets_init', 'agrilloc_register_about_author_widgets' );
function agrilloc_register_about_author_widgets() {
	register_widget( 'Agrilloc_About_Author_Widget' );
}
