...
if ( ! class_exists( 'Agrilloc_Custom_Posts_Widget' ) ) {

	class Agrilloc_Custom_Posts_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name			= esc_html__( 'Custom Posts', 'agrilloc' );
			$this->widget_description 	= esc_html__( 'Display custom posts your site.', 'agrilloc' );
			$this->widget_id			= apply_filters( 'agrilloc_custom_posts_widget_ID', 'widget-custom-posts' );
			$this->widget_cssclass		= apply_filters( 'agrilloc_custom_posts_widget_cssclass', 'widget-custom-posts custom-posts' );
			$this->utility				= agrilloc_utility()->utility;
			$this->settings 			= array(
				'title' => array(
					'type'				=> 'text',
					'value'				=> esc_html__( 'Custom Posts', 'agrilloc' ),
				...
				'terms_type' => array(
					...
					'options'			=> array(
						...
						'post_format'	=> array(
							'label'		=> esc_html__( 'Post Format', 'agrilloc' ),
				...
				'post_format' => array(
					...
					'options'			=> false,
					'label'				=> esc_html__( 'Select post format', 'agrilloc' ),
					'multiple'			=> true,
					'placeholder'		=> esc_html__( 'Select post format', 'agrilloc' ),
				...
				'posts_per_page' => array(
					...
					'max_value'			=> 50,
					'min_value'			=> 0,
					'label'				=> esc_html__( 'Posts count ( Set 0 to show all. )', 'agrilloc' ),
				),
				'post_offset' => array(
					'type'				=> 'stepper',
					'value'				=> '0',
				...
				'excerpt_length' => array(
					...
					'min_value'			=> '0',
					'step_value'		=> '1',
					'label'				=> esc_html__( 'Excerpt words length ( Set 0 to hide excerpt. )', 'agrilloc' ),
				),
				'meta_data' => array(
					'type'				=> 'checkbox',
					'value'				=> array(
					...
					'options'				=> array(
						'date'				=> esc_html__( 'Date', 'agrilloc' ),
						'author'			=> esc_html__( 'Author', 'agrilloc' ),
						'comment_count'		=> esc_html__( 'Comment count', 'agrilloc' ),
						'category'			=> esc_html__( 'Category', 'agrilloc' ),
						'post_tag'			=> esc_html__( 'Tag', 'agrilloc' ),
						'more_button'		=> esc_html__( 'More Button', 'agrilloc' ),
					),
					'label'				=> esc_html__( 'Display post meta data', 'agrilloc' ),
				),
				'button_text' => array(
					'type'				=> 'text',
					'value'				=> 'Read More',
					'label'				=> esc_html__( 'Post read more button label', 'agrilloc' ),
				),
			);
					...
					if ( $holder_view_dir ) {
						foreach ( $posts as $post ) {
							...
							$count = $this->utility->meta_data->get_comment_count( array(
								...
								'sufix'			=> _n_noop( '%s comment', '%s comments', 'agrilloc' ),
							) );
	...
	add_action( 'widgets_init', 'agrilloc_register_custom_posts_widget' );
	function agrilloc_register_custom_posts_widget() {
		register_widget( 'Agrilloc_Custom_Posts_Widget' );
	}
}
