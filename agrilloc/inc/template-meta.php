 ...
 * @return array
 */
function agrilloc_share_buttons( $context = 'loop', $args = array(), $config = array() ) {
	...
	if ( ! agrilloc_is_meta_visible( $meta, $context ) ) {
		return;
	}
	...
	$default_config = apply_filters( 'agrilloc_default_config_share_buttons', array(
	...
	$share_item_html = apply_filters( 'agrilloc_share_button_html',
	...
	foreach ( (array) $networks as $id => $network ) :
		...
		$share_buttons .= sprintf(
			...
			esc_html__( 'Share on ', 'agrilloc' ) . $network['name'],
	...
	printf(
		...
		'<div class="share-btns-main"><div class="share-btns__list %1$s">%2$s</div><div class="share-btns__show-icon"><i class="fa fa-share-alt"></i></div></div>',
